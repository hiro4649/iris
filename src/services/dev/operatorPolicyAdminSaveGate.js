...
const SCHEMA = "iris_operator_policy_admin_save_gate_v1";
...
const OPERATOR_POLICY_ADMIN_SAVE_GATE_FIELDS = new Set([
  "schema",
  "generated_at_ms",
  "save_status",
  "setting_id",
...
export function createOperatorPolicyAdminSaveGate({
  body = {},
  authContext = {},
  policyStore = null,
  auditLog = null,
  ...
  if (blockedReasons.length === 0) {
    ...
    const auditEntry = createOperatorPolicyAuditEntry({
      eventId: `operator_policy_save_${generatedAtMs}`,
    ...
    if (postgresWriteEnabled === true) {
      if (
        ...
        throw new ContractError(
          "operator policy admin save gate: postgres adapter is required when enabled"
        );
      }
      publicPostgresResult = postgresAdapter.persistApprovedOperatorPolicy(
        approvedRecord,
        auditEntry
      );
      if (typeof publicPostgresResult?.then === "function") {
        throw new ContractError(
          "operator policy admin save gate: async postgres adapters require private async gate"
        );
      }
      ...
      if (!postgresWritePerformed) {
        throw new ContractError(
          "operator policy admin save gate: postgres adapter did not persist"
        );
      }
    }
  }
  ...
  const gate = {
    ...
    boundary_policy: {
      ...
      no_postgres_write_by_default: true,
...
export function assertOperatorPolicyAdminSaveGateSafe(
  gate,
  context = "operator policy admin save gate"
) {
  ...
  for (const field of Object.keys(gate)) {
    if (!OPERATOR_POLICY_ADMIN_SAVE_GATE_FIELDS.has(field)) {
      throw new ContractError(`${context}: unexpected save gate field ${field}`);
    }
  }
  if (gate.postgres_write_performed !== false && gate.postgres_write_performed !== true) {
    throw new ContractError(`${context}: invalid postgres write state`);
  }
