...
} from "./gameplayLocalEnvProfile.js";

const URL_PATTERN = /\bhttps?:\/\//i;
const APPLY_MODES = new Set(["dry_run", "materialize"]);
const APPLY_STATUSES = new Set([
  "ready_to_create_gameplay_local_env_file",
  "ready_to_append_gameplay_local_env_names",
  "gameplay_local_env_file_created",
  "gameplay_local_env_names_appended",
  "no_missing_gameplay_local_env_names",
]);
...
const GAMEPLAY_LOCAL_ENV_APPLY_PLAN_FIELDS = new Set([
  "schema",
  "generated_at_ms",
  "target_stage_id",
...
export function createGameplayLocalEnvApplyPlan({
  cwd = process.cwd(),
  generatedAtMs = Date.now(),
  applyMode = "dry_run",
} = {}) {
  if (!APPLY_MODES.has(applyMode)) {
    throw new ContractError("gameplay local env apply plan: invalid apply mode");
  }

  const profile = createGameplayLocalEnvProfile({ generatedAtMs });
  assertGameplayLocalEnvProfileSafe(profile, "gameplay local env apply profile");
  const template = renderGameplayLocalEnvTemplate({ includeComments: false });
  ...
  const report = {
    schema: "iris_gameplay_local_env_apply_plan_v1",
    ...
    never_overwrite_existing_env: true,
    ...
    verification_scripts: {
      schema: "iris_gameplay_local_env_apply_plan_scripts_v1",
  ...
  assertGameplayLocalEnvApplyPlanSafe(report);
  return report;
}

export function assertGameplayLocalEnvApplyPlanSafe(
  report,
  context = "gameplay local env apply plan"
) {
  ...
  if (report.schema !== "iris_gameplay_local_env_apply_plan_v1") {
    throw new ContractError(`${context}: invalid schema`);
  }
  for (const field of Object.keys(report)) {
    if (!GAMEPLAY_LOCAL_ENV_APPLY_PLAN_FIELDS.has(field)) {
  ...
  if (report.target_stage_priority !== 4) {
  ...
  if (report.template_profile_status !== "ready_to_render_gameplay_local_env_template") {
  ...
  assertEnvNameList(report.missing_template_env_names, `${context}: missing env names`);
  assertEnvNameList(
...
function resolveApplyStatus({ applyMode, envFileExistsBefore, missingTemplateEnvNames }) {
  if (missingTemplateEnvNames.length === 0) return "no_missing_gameplay_local_env_names";
...
function appendMissingTemplate(
  ...
  const header = [
    "",
    "# IRIS gameplay local profile",
    "# Fill local/private vision and approved-control bridge values before live operation.",
    ...missingLines,
  ];
...
function assertVerificationScriptsSafe(scripts, context) {
  ...
  if (scripts.schema !== "iris_gameplay_local_env_apply_plan_scripts_v1") {
