...
function buildKnowledgeFixture(tmp) {
  const files = [
  ...
  for (const file of files) {
    ...
    fs.writeFileSync(path.join(tmp, file), 'fixture\n');
  }
  const map = {
    marker,
    harnessVersion: HARNESS_VERSION,
    safeSummaryOnly: true,
    sourceOfRecord: files,
    policyIndex: files.filter((file) => file.endsWith('_POLICY.md')),
  ...
  fs.writeFileSync(path.join(tmp, 'docs', 'process', 'CODEX_KNOWLEDGE_MAP.json'), JSON.stringify(map, null, 2));
}

function buildEvidencePackReportInCleanCwd(env) {
  return withTempCwd(() => buildEvidencePackReport(env));
}
...
export function buildV087SelfTestReport() {
  ...
  result = buildPromptGovernanceReport(prEnv({
    ...
    CODEX_PROMPT_EVAL_JSON: JSON.stringify({
      status: 'pass',
      cases: [
        { id: 'bugfix_skill_requires_reproduction' },
        { id: 'bugfix_skill_requires_root_cause' },
        { id: 'bugfix_skill_requires_verification' },
      ],
      safeSummaryOnly: true,
    }),
  })).promptGovernanceStatus;
  ...
  result = withTempCwd((tmp) => {
    buildKnowledgeFixture(tmp);
    return buildKnowledgeGovernanceReport().knowledgeGovernanceStatus;
  });
...
Stop condition: stop`,
  ...
  result = buildEvidencePackReportInCleanCwd(evidencePackEnv({
    CODEX_PR_COMMENTS: [
  ...
  result = buildPromptGovernanceReport(prEnv({
    ...
    CODEX_PROMPT_EVAL_JSON: JSON.stringify({
      status: 'pass',
      cases: [{ id: 'prompt_governance_policy_eval_current' }],
      safeSummaryOnly: true,
    }),
  })).promptGovernanceStatus;
  const knowledge = withTempCwd((tmp) => {
    buildKnowledgeFixture(tmp);
    return buildKnowledgeGovernanceReport(prEnv({ CODEX_PR_BODY: sourcePrBody() })).knowledgeGovernanceStatus;
  });
