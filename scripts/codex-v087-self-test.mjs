...
function isolatedEvidencePackReport(env) {
  const cwd = process.cwd();
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-v087-evidence-'));
  try {
    process.chdir(dir);
    return buildEvidencePackReport(env).evidencePackStatus;
  } finally {
    process.chdir(cwd);
  }
}
...
export function buildV087SelfTestReport() {
  ...
  result = buildPromptGovernanceReport(prEnv({
    ...
    CODEX_PROMPT_EVAL_SUITE_PATH: tempJson({
      cases: [
        { id: 'bugfix_skill_requires_reproduction', target: '.agents/skills/codex-bugfix/SKILL.md', mustContain: ['reproduction'], expectedStatus: 'pass', safeSummaryOnly: true },
        { id: 'bugfix_skill_requires_root_cause', target: '.agents/skills/codex-bugfix/SKILL.md', mustContain: ['root-cause'], expectedStatus: 'pass', safeSummaryOnly: true },
        { id: 'bugfix_skill_requires_verification', target: '.agents/skills/codex-bugfix/SKILL.md', mustContain: ['Verify'], expectedStatus: 'pass', safeSummaryOnly: true },
      ],
    }),
  })).promptGovernanceStatus;
  ...
  result = buildKnowledgeGovernanceReport({ ...baseEnv(), CODEX_KNOWLEDGE_MAP_PATH: tempJson(currentKnowledgeMap()) }).knowledgeGovernanceStatus;
...
Stop condition: stop`,
  ...
  result = isolatedEvidencePackReport(evidencePackEnv({
    CODEX_PR_COMMENTS: [
  ...
  result = buildPromptGovernanceReport(prEnv({
    ...
    CODEX_PROMPT_EVAL_SUITE_PATH: tempJson({
      cases: [
        { id: 'source_harness_governance_positive_fixture', target: 'scripts/codex-v087-self-test.mjs', mustContain: ['source harness'], expectedStatus: 'pass', safeSummaryOnly: true },
      ],
    }),
  })).promptGovernanceStatus;
  const knowledge = buildKnowledgeGovernanceReport(prEnv({ CODEX_PR_BODY: sourcePrBody(), CODEX_KNOWLEDGE_MAP_PATH: tempJson(currentKnowledgeMap()) })).knowledgeGovernanceStatus;
