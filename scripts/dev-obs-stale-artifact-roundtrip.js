...
const OBS_STALE_ARTIFACT_ROUNDTRIP_REPORT_FIELDS = new Set([
  "ok",
  "schema",
  "latest_manifest",
  "artifact_delivery",
...
const tempDir = mkdtempSync(join(tmpdir(), "iris-obs-stale-artifact-roundtrip-"));
const artifactDir = join(tempDir, "artifacts");
const renderManifestMaxAgeMs = 60_000;
const eventId = "obs-stale-artifact-fixture";
...
createRenderManifestFixture({
  ...
  staleRenderedAtMs: nowMs - renderManifestMaxAgeMs - 5000,
});
...
try {
  ...
  assert.equal(manifestReport.obs_pickup_status, "stale_artifact");
  assert.equal(manifestReport.obs_handoff_readiness_status, "waiting_for_fresh_render");
  assert.equal(summary.obs_pickup_ready, false);
  assert.equal(summary.obs_pickup_status, "stale_artifact");
  assert.equal(summary.obs_handoff_readiness_status, "waiting_for_fresh_render");
  assert.equal(summary.manifest_freshness_status, "fresh");
  assert.equal(summary.artifact_freshness_status_by_adapter.tts, "stale");
  assert.equal(summary.stale_artifact_rejected_for_obs_pickup_by_adapter.tts, true);
  ...
  assert.equal(summary.all_artifacts_fresh_for_pickup, false);
  assertNoUnsafeReportLeak(manifestReport, "latest render manifest stale artifact report");
  ...
  assert.equal(artifactStatuses.tts.error_kind, "stale_artifact");
  assert.equal(artifactStatuses.live2d.error_kind, "stale_artifact");
  assert.equal(artifactStatuses.subtitle.error_kind, "stale_artifact");
  assert.equal(artifactStatuses.tts.readiness_status, "waiting_for_fresh_render");
  assert.equal(artifactStatuses.live2d.readiness_status, "waiting_for_fresh_render");
  assert.equal(artifactStatuses.subtitle.readiness_status, "waiting_for_fresh_render");
  ...
  const report = {
    ok: true,
    schema: "iris_obs_stale_artifact_roundtrip_report_v1",
    latest_manifest: {
      ...
      stale_artifact_rejected_for_obs_pickup_by_adapter:
        summary.stale_artifact_rejected_for_obs_pickup_by_adapter,
    ...
    boundary_policy: {
      ...
      stale_artifact_guard_verified: true,
    },
  };
  assertObsStaleArtifactRoundtripReportSafe(report);
  assertNoUnsafeReportLeak(report, "OBS stale artifact roundtrip report");
...
function createRenderManifestFixture({ artifactDir, eventId, createdAtMs, staleRenderedAtMs }) {
  ...
  writeFileSync(
    ...
    "WEBVTT\n\n00:00.000 --> 00:01.000\nOBS stale artifact fixture subtitle\n",
    "utf8"
  );
  const manifest = {
    ...
    artifact_set: {
      tts: {
        ...
        rendered_at_ms: staleRenderedAtMs,
      },
      live2d: {
        adapter_kind: "live2d",
...
function assertObsStaleArtifactRoundtripReportSafe(report) {
  if (!report || typeof report !== "object" || Array.isArray(report)) {
    throw new Error("OBS stale artifact roundtrip report missing");
  }
  for (const field of Object.keys(report)) {
    if (!OBS_STALE_ARTIFACT_ROUNDTRIP_REPORT_FIELDS.has(field)) {
      throw new Error(`OBS stale artifact roundtrip unexpected report field ${field}`);
    }
  }
  if (
    report.ok !== true ||
    report.schema !== "iris_obs_stale_artifact_roundtrip_report_v1"
  ) {
    throw new Error("OBS stale artifact roundtrip status mismatch");
  }
  assertBoundaryPolicy(report.boundary_policy, [
    ...
    "no_commands",
    "no_secret_values",
    "stale_artifact_guard_verified",
  ], "OBS stale artifact roundtrip");
}
