...
export function buildV101SelfTestRegistrationReport(input = {}) {
  ...
  if (registeredStatusKeys) {
    if (!registeredStatusKeys.includes('v101SelfTestStatus')) reasons.push('v101_self_test_missing');
  } else if (!readText('scripts/codex-local-quality-gate.mjs')?.includes('v101SelfTestStatus')) reasons.push('v101_self_test_missing');
  if (registeredSelfTests) {
    if (!registeredSelfTests.includes('codex-v101-self-test.mjs')) reasons.push('v101_self_test_missing');
  } else if (!readText('CODEX_SOURCE_HARNESS_MANIFEST.json')?.includes('codex-v101-self-test.mjs')) reasons.push('v101_self_test_missing');
