...
function isolatedEvidencePackStatus(env) {
  const cwd = process.cwd();
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-v090-evidence-'));
  try {
    process.chdir(dir);
    return buildEvidencePackReport(env).evidencePackStatus.status;
  } finally {
    process.chdir(cwd);
    fs.rmSync(dir, { recursive: true, force: true });
  }
...
function buildV090SelfTestReport() {
  ...
  assertCase('workflow_dispatch_main_does_not_require_evidence_pack', isolatedEvidencePackStatus({ CODEX_EVENT_NAME: 'workflow_dispatch', CODEX_HARNESS_SOURCE_REPO: '1', CODEX_EVIDENCE_PACK_STRICT: '0' }) === 'not_applicable', failures, cases, 'pass', []);
