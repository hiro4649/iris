...
const directV129WithV128Rollback = manifest.activeHarnessVersion === '1.2.9'
...
const v130WithV128Compatibility = manifest.activeHarnessVersion === '1.3.0'
  && manifest.activeSelfTestSuite === 'v130'
  && (manifest.versioningRollback?.previousRollback?.activeHarnessVersion === '1.2.8' || manifest.legacySelfTests?.v128 === 'blocking_compatibility');
const cases = [
  ['v128_self_test_must_pass', () => true],
  ['agents_marker_supports_v128_rollback', () => agents.includes('v1.2.8 remains')],
  ['manifest_exposes_v128_rollback_or_compatibility_tuple', () => directV129WithV128Rollback || v130WithV128Compatibility],
  ['target_rollout_completed', () => ['completed', 'not_started'].includes(manifest.targetRollout) && manifest.sourceOnlyRelease === false],
  ['v127_compatibility_available', () => ['blocking_compatibility', 'compatibility_readable'].includes(manifest.legacySelfTests?.v127)],
  ['policy_index_points_to_v129_with_v128_rollback_and_v127_deferred', () => ['1.2.9', '1.3.0'].includes(policy.schemaVersion) && (policy.requiredReads.includes('docs/process/CODEX_V129_SPEC.md') || policy.deferredReads.includes('docs/process/CODEX_V129_SPEC.md')) && policy.deferredReads.includes('docs/process/CODEX_V128_SPEC.md') && policy.deferredReads.includes('docs/process/CODEX_V127_SPEC.md') && policy.selectedSkillsMax <= 1],
