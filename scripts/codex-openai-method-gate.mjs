...
const profileTemplateMarker = 'CODEX_QUALITY_HARNESS_FILE v1.0.8';
const agentsDoctrineMarker = 'CODEX_QUALITY_HARNESS_FILE v1.1.7';
...
function inspectSupportFiles(policy) {
  ...
  const shapeRules = {
    ...
    prTemplate: [/Owner Summary/i, /Evidence Source/i, /Risk And Readiness/i],
    agents: [/CODEX_OPENAI_CODEX_METHOD_POLICY\.md|code_review\.md/i],
  };
  const expectedMarkers = {
    prTemplate: [profileTemplateMarker],
    agents: [agentsDoctrineMarker],
  };
  for (const [key, file] of Object.entries(managedPaths)) {
    ...
    if (text === null) failures.push(`${key}=missing`);
      ...
      const allowedMarkers = expectedMarkers[key] || [marker];
      if (!allowedMarkers.some((allowedMarker) => text.includes(allowedMarker))) failures.push(`${key}=marker_missing`);
