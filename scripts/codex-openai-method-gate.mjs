...
const harnessMarkerPattern = /CODEX_QUALITY_HARNESS_FILE v\d+\.\d+\.\d+/;
...
function inspectSupportFiles(policy) {
  ...
  const shapeRules = {
    ...
    prTemplate: [/(Codex Method Compliance|Owner Summary)/i, /(Code review status:|Review independence:|Task mode:)/i],
  ...
  for (const [key, file] of Object.entries(managedPaths)) {
    ...
    if (text === null) failures.push(`${key}=missing`);
      ...
      if (!harnessMarkerPattern.test(text)) failures.push(`${key}=marker_missing`);
  ...
  if (!harnessMarkerPattern.test(String(policy.marker || ''))) failures.push('policyJson=marker_mismatch');
  return {
