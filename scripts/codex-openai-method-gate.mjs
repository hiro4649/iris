...
const supportMarkerPattern = /CODEX_QUALITY_HARNESS_FILE v\d+\.\d+\.\d+/;
...
function inspectSupportFiles(policy) {
  ...
  const shapeRules = {
    ...
    prTemplate: [/(Codex Method Compliance|Owner Summary)/i, /(Code review status:|Evidence Source)/i],
  ...
  for (const [key, file] of Object.entries(managedPaths)) {
    ...
    if (text === null) failures.push(`${key}=missing`);
      ...
      if (!supportMarkerPattern.test(text)) failures.push(`${key}=marker_missing`);
  ...
  if (!supportMarkerPattern.test(String(policy.marker || ''))) failures.push('policyJson=marker_mismatch');
  return {
