...
function diffBaseCandidates() {
  ...
  add(process.env.CODEX_PR_BASE_SHA);
  add(process.env.CODEX_PR_BASE_REF);
...
function envExampleSafeKeyOnlyStatus(policy, file) {
  ...
  const target = normalizePath(file);
  if (cfg.enabled !== true || target !== configuredPath) return { safe: false, reason: 'not_configured_exception_path' };
  ...
  for (const line of addedContent) {
    ...
    if (looksSecretLike(trimmed) || looksEndpointLike(trimmed) || /(?:[A-Za-z]:\\|\/(?:home|Users)\/|https?:\/\/|\b(?:curl|bash|sh|powershell)\b)/i.test(trimmed)) {
