...
const directV129 = manifest.activeHarnessVersion === '1.2.9'
...
const v130WithV129Rollback = manifest.activeHarnessVersion === '1.3.0'
  && manifest.activeSelfTestSuite === 'v130'
  && manifest.versioningRollback?.activeHarnessVersion === '1.2.9'
  && manifest.versioningRollback?.activeSelfTestSuite === 'v129'
  && manifest.versioningRollback?.activeSelfTestStatusKey === 'v129SelfTestStatus'
  && manifest.versioningRollback?.rollbackAvailable === true;
const cases = [
  ['v129_self_test_must_pass', () => true],
  ['agents_marker_is_v129', () => agents.includes('CODEX_QUALITY_HARNESS_FILE v1.2.9') || agents.includes('v1.2.9 remains available as immediate rollback')],
  ['manifest_active_tuple_is_v129_or_v130_with_v129_rollback', () => directV129 || v130WithV129Rollback],
  ...
  ['v128_rollback_tuple_available', () => (manifest.versioningRollback?.activeHarnessVersion === '1.2.8' || manifest.versioningRollback?.previousRollback?.activeHarnessVersion === '1.2.8') && ['blocking_compatibility_rollback', 'blocking_compatibility'].includes(manifest.legacySelfTests?.v128)],
  ['v127_compatibility_available', () => ['blocking_compatibility', 'compatibility_readable'].includes(manifest.legacySelfTests?.v127)],
  ['policy_reads_v129_and_defers_legacy', () => (policy.requiredReads.includes('docs/process/CODEX_V129_SPEC.md') || policy.deferredReads.includes('docs/process/CODEX_V129_SPEC.md')) && policy.deferredReads.includes('docs/process/CODEX_V128_SPEC.md') && policy.deferredReads.includes('docs/process/CODEX_V127_SPEC.md')],
  ['token_budgets_remain_restricted', () => (router.routineColdArtifactRead ?? 0) === 0 && (router.routineSelectedSkillMax ?? 0) <= 1 && (router.routineReviewerFanout ?? 0) === 0 && (router.routineOwnerInterruptMax ?? 0) === 0 && (policyRouter.routineColdArtifactRead ?? 0) === 0 && (policyRouter.routineSelectedSkillMax ?? policy.selectedSkillsMax ?? 0) <= 1],
  ['target_safety_preserved', () => router['irisSafetyPreserved'] === true],
