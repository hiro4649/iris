...
const engineServer = createServer(async (request, response) => {
  if (request.url === "/tts-engine") {
    ...
    response.end(
      JSON.stringify({
        audio_base64: Buffer.from("not a real wav even though mime says wav", "utf8").toString(
          "base64"
        ),
...
const tempDir = mkdtempSync(join(tmpdir(), "iris-engine-invalid-audio-roundtrip-"));
...
try {
  ...
  const publicReport = {
    ok:
      ...
      workerReport.receipts?.[0]?.error_kind === "invalid_audio_response" &&
    ...
    boundary_policy: {
      invalid_audio_content_rejected_before_artifact_write: true,
  ...
  if (
    serialized.includes("not a real wav") ||
    serialized.includes("IRIS bridge fixture voice check") ||
    serialized.includes('"audio_base64"') ||
