...
function withCurrentVersionLineageFixture(fn) {
  ...
  try {
    ...
    writeFixtureFile(root, 'README.md', `Version: v${HARNESS_VERSION}\n`);
    writeFixtureFile(root, 'CODEX_SOURCE_HARNESS_MANIFEST.json', JSON.stringify(manifest));
    writeFixtureFile(root, 'docs/process/CODEX_KNOWLEDGE_MAP.json', JSON.stringify({ marker, harnessVersion: HARNESS_VERSION, safeSummaryOnly: true }));
...
function buildV092SelfTestReport() {
  ...
  let report = withCurrentVersionLineageFixture(() => buildVersionLineageReport({ CODEX_HARNESS_SOURCE_REPO: '1', CODEX_HARNESS_MODE: 'core' }));
