...
Priority1 production blocker resolution is governed by `docs/iris/IRIS_PRODUCTION_REAL_EVIDENCE_MANIFEST.md`, the live handoff bundle contract in `docs/iris/IRIS_PRODUCTION_LIVE_HANDOFF_BUNDLE.md`, and the production go/no-go package contract in `docs/iris/IRIS_PRODUCTION_GO_NOGO_PACKAGE.md`.
...
Generating a production go/no-go package is not production go, does not perform real operations, does not collect fresh evidence, and does not confirm owner approval.
